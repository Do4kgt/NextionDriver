...
#pragma GCC diagnostic pop
        ...
        if (user>=0) {
            sprintf(TXbuffer,"t39.txt=\"%s\"",users[user].data1);
            sendCommand(TXbuffer);
            sprintf(TXbuffer,"t40.txt=\"%s\"",users[user].data2);
            sendCommand(TXbuffer);
            sprintf(TXbuffer,"t41.txt=\"%s\"",users[user].data3);
            sendCommand(TXbuffer);
            sprintf(TXbuffer,"t42.txt=\"%s\"",users[user].data4);
            sendCommand(TXbuffer);
            sprintf(TXbuffer,"t43.txt=\"%s\"",users[user].data5);
            sendCommand(TXbuffer);
        } else {
            sprintf(TXbuffer,"t40.txt=\"DMRID %s\"",text);
            sendCommand(TXbuffer);
            sprintf(TXbuffer,"t41.txt=\"Not found in\"");
            sendCommand(TXbuffer);
            sprintf(TXbuffer,"t42.txt=\"%s\"",usersFile);
            sendCommand(TXbuffer);
        }
